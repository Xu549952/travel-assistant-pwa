...
# 用法: .\lhci-local.ps1 [autorun|collect|assert|serve]
#
# 前置条件:
...
switch ($Command) {
    "autorun" {
        ...
        if ($exitCode -eq 0) {
            Write-Host "`n[PASS] 所有阈值检查通过!" -ForegroundColor Green
        } else {
            Write-Host "`n[FAIL] 存在未通过阈值的项目，请查看上方报告" -ForegroundColor Red
        }

        # 检查是否有报告生成
        $reportDir = "$ProjectDir\.lighthouseci"
        if (Test-Path $reportDir) {
            Write-Host "`n[INFO] HTML 报告位置: $reportDir" -ForegroundColor Cyan
            $htmlReports = Get-ChildItem "$reportDir\*.html" -ErrorAction SilentlyContinue
            if ($htmlReports) {
                Write-Host "[INFO] 可用 HTML 报告:" -ForegroundColor Cyan
